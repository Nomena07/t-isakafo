...
// ── Initialiser WebSocket (skip on Vercel) ────────────────────────────────
if (!isVercel) {
  ws.init(server);
}
...
// ── Servir les images uploadées (skip on Vercel) ────────────────────────
if (!isVercel) {
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', time: new Date().toISOString(), version: '1.0.0', platform: isVercel ? 'vercel' : 'local' });
});

// ── Toutes les autres routes → frontend SPA (skip on Vercel) ────────────
if (!isVercel) {
...
// ── Exporter pour Vercel / Démarrer en local ─────────────────────────────
if (isVercel) {
  module.exports = app;
} else {
